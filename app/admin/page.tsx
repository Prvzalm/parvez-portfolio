...
type Trace = { id: string; name: string; image_url: string; cloudinary_public_id?: string; created_at: string; active: boolean; moderation_status: string; user_agent?: string; ip_hash?: string; timezone?: string; screen?: string; language?: string; platform?: string; referer?: string };
...
export default function AdminPage() {
  ...
  return <main className="admin-page">
    ...
    {loaded ? <div className="admin-grid">{traces.map((trace) => <article className={`admin-trace ${trace.active ? "" : "is-inactive"}`} key={trace.id}><img src={trace.image_url} alt={`Trace by ${trace.name}`} /><div><strong>{trace.name} · {trace.active ? "ACTIVE" : "INACTIVE"}</strong><small>{new Date(trace.created_at).toLocaleString("en-GB")}</small><small>{trace.timezone ?? "Timezone unavailable"} · {trace.screen ?? "Screen unavailable"}</small><small title={trace.user_agent}>{trace.platform ?? "Platform unavailable"} · {trace.language ?? "Language unavailable"}</small><button type="button" onClick={() => void toggleActive(trace)}>{trace.active ? "Deactivate" : "Activate"}</button><button type="button" onClick={() => void remove(trace)}>Delete permanently</button></div></article>)}</div> : null}
  </main>;
}
